...
import michal.malek.gateway.configuration.Carousel;
import michal.malek.gateway.utills.JwtUtil;
    ...
    public GatewayFilter apply(Config config) {
        return ((exchange, chain) -> {
            if (validator.isSecure.test(exchange.getRequest())) {
                ...
                HttpCookie authCookie = exchange.getRequest().getCookies().get("token").get(0);
